...
def test_submission_transport_requires_typed_context():
...
def test_unsupported_portal_cannot_reach_final_click_transport():
    context=SubmissionContext("r",1,"p","e","WORKDAY","http://127.0.0.1/workday","f")
    result=ApplicationBrowserService().submit_final_url(context)
...
def test_stale_rehydrated_identity_fails_closed_before_browser_launch():
    context=SubmissionContext("r",1,"p","e","GREENHOUSE","http://127.0.0.1/app","fingerprint")
    ...
    result=ApplicationBrowserService(package_service=Packages(),review_service=Reviews()).submit_final_url(context)
