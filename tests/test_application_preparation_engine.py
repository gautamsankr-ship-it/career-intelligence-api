...
def test_multi_page_preparation_batches_required_exceptions(tmp_path):
    engine=ApplicationPreparationEngine(session_dir=tmp_path)
...
def test_session_persistence_contains_no_secrets(tmp_path):
    engine=ApplicationPreparationEngine(session_dir=tmp_path); session=engine.prepare_pages([P3],vacancy())
