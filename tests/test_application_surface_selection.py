...
def _select(url, expected_portal="GREENHOUSE"):
    async def run():
...
def test_a_direct_greenhouse_page_is_selected_without_iframe_search():
    ats = LocalATS(); url = ats.start()
    try:
        status, surface_is_page, html, _ = _select(url)
...
def test_b_trusted_greenhouse_iframe_is_selected():
    wrapper = LocalGreenhouseIframeWrapper("single"); url = wrapper.start()
    try:
        status, surface_is_page, html, final_page_url = _select(url)
...
def test_c_unrelated_iframe_is_ignored_in_favor_of_trusted_greenhouse_iframe():
...
def test_d_missing_application_surface_fails_closed():
    wrapper = LocalGreenhouseIframeWrapper("none"); url = wrapper.start()
    try:
        status, _, surface_html, _ = _select(url)
        assert status == "APPLICATION_SURFACE_NOT_FOUND" and surface_html is None
    finally:
        wrapper.close()


def test_e_ambiguous_trusted_iframes_fail_closed_without_selection():
...
def test_f_captcha_iframe_is_never_selected_as_application_surface():
...
def test_g_stable_route_is_not_replaced_by_the_selected_iframe():
    wrapper = LocalGreenhouseIframeWrapper("single"); url = wrapper.start()
    stable_route = {"application_url": url}
    try:
        status, surface_is_page, _, final_page_url = _select(url)
        assert status == "TRUSTED_IFRAME" and not surface_is_page
